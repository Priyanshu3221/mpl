...
const shell = (page: React.ReactNode, permission?: (typeof PERMISSIONS)[keyof typeof PERMISSIONS]) => <ProtectedRoute>{permission ? <RoleProtectedRoute permission={permission}><AppShell>{page}</AppShell></RoleProtectedRoute> : <AppShell>{page}</AppShell>}</ProtectedRoute>;
...
export default function App() {
  return <Switch>
    ...
    <Route path="/flags/collusion">{shell(<RouteStub title="Collusion review"/>, PERMISSIONS.REVIEW_FLAGS)}</Route>
    <Route path="/reports">{shell(<RouteStub title="Reports"/>, PERMISSIONS.VIEW_REPORTS)}</Route>
    <Route path="/notifications">{shell(<RouteStub title="Notifications"/>)}</Route>
    <Route path="/profile">{shell(<RouteStub title="Profile"/>)}</Route>
    <Route path="/users">{shell(<RouteStub title="User management"/>, PERMISSIONS.MANAGE_USERS)}</Route>
    <Route path="/audit">{shell(<RouteStub title="Audit trail"/>, PERMISSIONS.VIEW_AUDIT)}</Route>
    <Route path="/compliance">{shell(<RouteStub title="Compliance"/>)}</Route>
    <Route path="/accessibility">{shell(<RouteStub title="Accessibility"/>)}</Route>
