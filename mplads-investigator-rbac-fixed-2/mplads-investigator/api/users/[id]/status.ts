...
import { clerkAdminConfigured, setUserActiveInClerk } from "../../../server/clerkUsers";
import { can, PERMISSIONS } from "../../../shared/roles";

/**
 * Vercel equivalent of PATCH /api/users/:id/status in server/index.ts.
 * Only an Admin's own verified session token can reach the Clerk ban/unban
 * write below; nothing in the request body influences who the caller is.
 */
export default async function handler(req: any, res: any) {
  ...
  try {
    await setUserActiveInClerk(Array.isArray(targetUserId) ? targetUserId[0] : targetUserId, status === "Active");
  } catch (error) {
  ...
  return res.json({ authorized: true, actorRole: principal.role, userId: targetUserId, status });
}
